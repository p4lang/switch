...
static switch_meter_mode_t
sai_meter_mode_to_switch_meter_mode(
...
static switch_meter_color_source_t
...
static switch_meter_type_t
sai_meter_type_to_switch_meter_type(
...
sai_status_t sai_create_policer(
    ...
    *policer_id = switch_api_meter_create(device, &api_meter_info);
    status = (*policer_id == SWITCH_API_INVALID_HANDLE) ?
             SAI_STATUS_FAILURE :
             SAI_STATUS_SUCCESS;
    ...
    for (index = 0; index < number_of_counters; index++) {
        switch (counter_ids[index]) {
            ...
            case SAI_POLICER_STAT_ATTR_BYTES:
                counters[index] =
                             switch_counters[SWITCH_METER_STATS_GREEEN].num_bytes +
    ...
    _Out_ uint64_t* counters)
{
    ...
    switch_counters = SAI_MALLOC(sizeof(switch_counter_t) * SWITCH_METER_STATS_MAX);
    if (!switch_counters) {
        status = SAI_STATUS_NO_MEMORY;
        SAI_LOG_ERROR("failed to get meter stats %lx: %s",
                      policer_id, sai_status_to_string(status));
        return status;
    }

    meter_stat_ids = SAI_MALLOC(sizeof(switch_meter_stats_t) * SWITCH_METER_STATS_MAX);
    if (!meter_stat_ids) {
        status = SAI_STATUS_NO_MEMORY;
        SAI_LOG_ERROR("failed to get meter stats %lx: %s",
                      policer_id, sai_status_to_string(status));
    ...
    switch_status = switch_api_meter_stats_get(
                             device,
                             policer_id,

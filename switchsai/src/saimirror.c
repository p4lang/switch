...
static switch_mirror_type_t sai_session_to_switch_session(_In_ sai_mirror_type_t
                                                              mirror_type) {
  switch (mirror_type) {
...
sai_status_t sai_create_mirror_session(_Out_ sai_object_id_t *session_id,
  ...
  *session_id = (sai_object_id_t)switch_api_mirror_session_create(
      device, &api_mirror_info);
  status = (*session_id == SWITCH_API_INVALID_HANDLE) ? SAI_STATUS_FAILURE
                                                      : SAI_STATUS_SUCCESS;

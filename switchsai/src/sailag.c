...
/*
    ...
    \return Success: SAI_STATUS_SUCCESS
            Failure: Failure status code on error
*/
sai_status_t sai_set_lag_attribute(
        _In_ sai_object_id_t lag_id,
        _In_ const sai_attribute_t *attr) {
...
/*
    ...
    \return Success: SAI_STATUS_SUCCESS
            Failure: Failure status code on error
*/
sai_status_t sai_get_lag_attribute(
        _In_ sai_object_id_t lag_id,
        _In_ uint32_t attr_count,
...
sai_status_t sai_create_lag_member(
    ...
    *lag_member_id = switch_api_lag_member_create(device,
                        (switch_handle_t) lag_id,
                        direction,
                        port_id);
    status = (*lag_member_id == SWITCH_API_INVALID_HANDLE) ?
             SAI_STATUS_FAILURE :
             SAI_STATUS_SUCCESS;
...
/*
    ...
    \return Success: SAI_STATUS_SUCCESS
            Failure: Failure status code on error
*/
sai_status_t sai_remove_lag_member(
    ...
    SAI_ASSERT(sai_object_type_query(lag_member_id) == SAI_OBJECT_TYPE_LAG_MEMBER);

    switch_status = switch_api_lag_member_remove(device, 
                                (switch_handle_t) lag_member_id);
    status = sai_switch_status_to_sai_status(switch_status);
    if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to remove lag member %lx : %s",
                      lag_member_id, sai_status_to_string(status));
    }
...
/*
    ...
    \return Success: SAI_STATUS_SUCCESS
            Failure: Failure status code on error
*/
sai_status_t sai_set_lag_member_attribute(
        _In_ sai_object_id_t  lag_member_id,
        _In_ const sai_attribute_t *attr) {
    SAI_LOG_ENTER();
...
sai_status_t sai_get_lag_member_attribute(
        _In_ sai_object_id_t lag_member_id,
        _In_ uint32_t attr_count,
        _Inout_ sai_attribute_t *attr_list) {
    SAI_LOG_ENTER();

...
static void sai_log_packet(_In_ int port_num,
  ...
  sprintf(log_pkt,
          "Packet in on port %d length %d; first bytes:\n",
          port_num,
          length);
  for (i = 0; i < 32; i++) {
    if (i && ((i % 4) == 0)) {
...
static void sai_transmit_packet(_In_ p4_port_t egress,
                                _In_ void *pkt,
                                _In_ int len) {
...
static void sai_receive_packet(_In_ int port_num,
                               _In_ const char *buffer,
                               _In_ int length) {
  if (log_level >= P4_LOG_LEVEL_TRACE) {
...
static sai_status_t sai_load_config(_In_ char *fname,

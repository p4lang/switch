...
typedef struct sai_attribute_param_check_create_s sai_attribute_param_check_create_t;
...
int sai_param_check_insert(int api, sai_attribute_param_api_type_t api_type,
    sai_attr_id_t id, sai_attribute_value_t *def, sai_attribute_value_t *min,
    sai_attribute_value_t *max, sai_attribute_param_type_e type, bool mandatory)
{
    ...
    if(p) {
        ...
        if(api_type == SAI_ATTRIBUTE_PARAM_API_TYPE_CREATE) {
            ...
            // insert into list
            if(mandatory) {
                tommy_list_insert_tail(&(sai_param_validation[api].create.mandatory_list), &(p->node), p);
            }
            else {
                tommy_list_insert_tail(&(sai_param_validation[api].create.possible_list), &(p->node), p);
            }
        }
...
int sai_param_check(int api, sai_attribute_param_api_type_t api_type,
...
static char *sai_param_json_file_get(sai_api_id_t api_id)
{
    switch (api_id) {
        ...
        case SAI_API_VIRTUAL_ROUTER:
            return "sairouter.json"
        case SAI_API_ROUTE:
            return "sairoute.json"
        case SAI_API_NEXT_HOP:
            return "sainextop.json"
        case SAI_API_NEXT_HOP_GROUP:
            return "sainexthopgroup.json"
        case SAI_API_ROUTER_INTERFACE:
            return "sairouterinterface.json"
        case SAI_API_NEIGHBOR:
            return "saineighbor.json"
        case SAI_API_ACL:
            return "saiacl.json"
        case SAI_API_HOST_INTERFACE:
            return "saihostintf.json"
        case SAI_API_MIRROR:
            return "saimirror.json"
        case SAI_API_STP:
            return "saistp.json"
        case SAI_API_LAG:
            return "sailag.json"
...
static sai_param_rw_type_t
sai_param_rw_type_get(
        _In_ char *rw_type) {
...
static sai_param_data_type_t
sai_param_data_type_get(
        _In_ char *data_type) {
...
sai_status_t sai_param_parse_attributes(
        _In_ cJSON *attr_object
        _In_ sai_param_attribute_info_t *attr_info)
{
        ...
        _Out_ sai_param_module_info_t *module_info)
{
    ...
    while (attributes_object) {
        ...
        while (attr_info_object) {
            if (attr_info_object->type == cJSON_String) {
                ...
                if (!strcmp(attr_info_object->string, "attribute_list")) {
                    status = sai_param_parse_attributes(attr_info_object->child, attr_info);
        ...
        tommy_list_insert_tail(&(module_info->attribute_info_list), &(attr_info->node), attr_info);
        attributes_object = attributes_object->next;
    }
    return SAI_STATUS_SUCCESS;
}

sai_status_t sai_param_parse_json(
        _In_ sai_api_id_t api_id, 
        _In_ char *file_name)
{

...
sai_status_t sai_create_next_hop_entry(_Out_ sai_object_id_t *next_hop_id,
  ...
  for (index = 0; index < attr_count; index++) {
    ...
    switch (attribute->id) {
      ...
      case SAI_NEXT_HOP_ATTR_IP:
        assert(nhtype == SAI_NEXT_HOP_TYPE_IP);
  ...
  *next_hop_id = (sai_object_id_t)switch_api_nhop_create(device, &nhop_key);
  status = (*next_hop_id == SWITCH_API_INVALID_HANDLE) ? SAI_STATUS_FAILURE
                                                       : SAI_STATUS_SUCCESS;
...
sai_status_t sai_set_next_hop_entry_attribute(
  ...
  if ((switch_status =
           switch_api_nhop_get(device, (switch_handle_t)next_hop_id, &key)) !=
  ...
  switch (attr->id) {
    ...
    case SAI_NEXT_HOP_ATTR_IP:
      status =
          sai_ip_addr_to_switch_ip_addr(&attr->value.ipaddr, &key->ip_addr);
      break;
    case SAI_NEXT_HOP_ATTR_ROUTER_INTERFACE_ID:

...
static void sai_fdb_entry_parse(const sai_fdb_entry_t *fdb_entry,
...
*    Failure status code on error
*/
sai_status_t sai_flush_fdb_entries(_In_ uint32_t attr_count,
                                   _In_ const sai_attribute_t *attr_list) {
  SAI_LOG_ENTER();
...
static void sai_mac_learn_notify_cb(switch_api_mac_entry_t *mac_entry) {
  SAI_LOG_ENTER();
...
static void sai_mac_age_notify_cb(switch_api_mac_entry_t *mac_entry) {
  if (!sai_switch_notifications.on_fdb_event) {
    return;
  }

...
static void
process_packet_from_user(int intf) {
    int ret, fd, i;
    char buf[2000];
    ...
    while((ret = read(fd, buf, sizeof(buf))) > 0) {
        ...
        if (switchlink_send_packet(buf, ret, g_intf[intf].port_id) != 0) {
            NL_LOG_ERROR(("Tx: Sending %d bytes from %s failed\n",
                          ret, g_intf[intf].name));
            continue;
        }
    }
...
packet_driver_init() {
    ...
    for (i = 0; i < num_ports; i++) {
        if (strncmp(port_map[i].name, SWITCHLINK_CPU_INTERFACE_NAME,
                    SWITCHLINK_INTERFACE_NAME_LEN_MAX) == 0) {
            continue;
        }
        strncpy(g_intf[g_num_intfs].name, port_map[i].name,
...
static void *
switchlink_packet_driver_main(void *args) {
    int i;
...
void
switchlink_packet_driver_update_mac(char *link_name, uint8_t *lladdr) {
    int i;
    for (i = 0; i < g_num_intfs; i++) {
        if (strncmp(g_intf[i].name, link_name,
                    SWITCHLINK_INTERFACE_NAME_LEN_MAX) == 0) {
    ...
    for (i = 0; i < g_num_intfs; i++) {
        if (g_intf[i].port_id == port_id) {
            if (write(g_intf[i].fd, buf, buf_size) < 0) {
                NL_LOG_ERROR(("Rx: Sending %d bytes to %s failed\n",
...
switchlink_packet_driver_init() {
    return pthread_create(&switchlink_packet_driver_thread, NULL,
                          switchlink_packet_driver_main, NULL);
}

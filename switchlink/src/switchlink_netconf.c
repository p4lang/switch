...
void
process_netconf_msg(struct nlmsghdr *nlmsg, int type) {
    ...
    if (status != SWITCHLINK_DB_STATUS_SUCCESS) {
        NL_LOG_DEBUG(("netconf: switchlink_db_interface_get_info "
                      "failed\n"));
        return;
    }
    ...
    if (ncmsg->ncm_family == AF_INET) {
        if (forwarding_valid &&
            ...
            switchlink_interface_forwarding_update(ifinfo.intf_h,
                                                   ncmsg->ncm_family,
                                                   forwarding);
            ifinfo.flags.ipv4_unicast_enabled = forwarding;
            update_intf = true;
        ...
        if (mc_forwarding_valid &&
            ...
            switchlink_interface_mc_forwarding_update(ifinfo.intf_h,
                                                      ncmsg->ncm_family,
                                                      mc_forwarding);
            ifinfo.flags.ipv4_multicast_enabled = mc_forwarding;
            update_intf = true;
        ...
        if (rp_filter_valid && (ifinfo.flags.ipv4_urpf_mode != rp_filter)) {
            switchlink_interface_urpf_mode_update(ifinfo.intf_h,
                                                  ncmsg->ncm_family,
                                                  rp_filter);

...
static void
switchlink_mdb_mroute_notify(switchlink_db_mroute_info_t *mroute_info) {
    switchlink_mroute_delete(mroute_info);
    switchlink_mroute_create(mroute_info);
}

static void
process_mdb_entry(int type, int ifindex, struct br_mdb_entry *bre) {
    ...
    if (ntohs(bre->addr.proto) == ETH_P_IP) {
        ...
        memcpy(&(mdb_info.grp_ip.ip.v6addr), &(bre->addr.u.ip6),
...
process_mdb_msg(struct nlmsghdr *nlmsg, int type) {
    ...
    NL_LOG_DEBUG(("%smdb: family = %d, ifindex = %d\n",
                  ...
                  brp_msg->family, brp_msg->ifindex));
                    ...
                    switch (nest_attr_type) {
                        case MDBA_MDB_ENTRY:
                            ...
                            if (nla_type(info_attr) == MDBA_MDB_ENTRY_INFO) {
                                bre =
                                    (struct br_mdb_entry *)nla_data(info_attr);

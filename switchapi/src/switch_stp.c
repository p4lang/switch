...
switch_handle_t switch_stg_handle_create() {
  switch_handle_t stg_handle;
  _switch_handle_create(SWITCH_HANDLE_TYPE_STP,
                        switch_stp_info_t,
...
void switch_stg_handle_delete(switch_handle_t stg_handle) {
  _switch_handle_delete(
      switch_stp_info_t, switch_stp_instance_array, stg_handle);
}

switch_stp_info_t *switch_api_stp_get_internal(switch_handle_t stg_handle) {
  switch_stp_info_t *stp_info = NULL;
  _switch_handle_get(
      switch_stp_info_t, switch_stp_instance_array, stg_handle, stp_info);
  return stp_info;
}
...
switch_status_t switch_api_stp_group_delete(switch_device_t device,
  ...
  while (node) {
    ...
    node = node->next;
    switch_api_stp_port_state_set(device,
                                  stg_handle,
  ...
  while (node) {
    ...
    node = node->next;
    switch_api_stp_group_vlans_remove(
        device, stg_handle, 1, &vlan_entry->bd_handle);
  }
...
switch_status_t switch_api_stp_group_vlans_add(switch_device_t device,
  ...
  for (count = 0; count < vlan_count; count++) {
    ...
    tommy_list_insert_head(
        &(stp_info->vlan_list), &(vlan_entry->node), vlan_entry);
  }
  return SWITCH_STATUS_SUCCESS;
}

switch_status_t switch_api_stp_group_vlans_remove(
...
switch_status_t switch_api_stp_port_state_set(switch_device_t device,
  ...
  if (state == SWITCH_PORT_STP_STATE_NONE) {
    ...
    if (!node) {
      ...
      port_entry->intf_state = state;
      tommy_list_insert_head(
          &(stp_info->port_list), &(port_entry->node), port_entry);
    }
    ...
    if (new_entry) {
      ...
      status =
          switch_pd_spanning_tree_table_update_entry(device,
                                                     handle_to_id(stg_handle),
                                                     intf_info->ifindex,
                                                     port_entry->intf_state,
                                                     port_entry->hw_entry);
...
switch_status_t switch_api_stp_group_print_entry(switch_handle_t stg_handle) {
  switch_stp_info_t *stp_info = NULL;
  switch_stp_vlan_entry_t *vlan_entry = NULL;
  switch_stp_port_entry_t *port_entry = NULL;
  tommy_node *node = NULL;
...
switch_status_t switch_api_stp_group_print_all(void) {
  ...
  while (stp_handle) {
    ...
    switch_handle_get_next(
        switch_stp_instance_array, stp_handle, next_stp_handle);

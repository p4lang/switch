...
void
switch_sflow_handle_delete(switch_handle_t sflow_handle)
{
    _switch_handle_delete(switch_sflow_info_t,
                          switch_sflow_array,
                          sflow_handle);
}

switch_sflow_info_t *
switch_sflow_info_get(switch_handle_t sflow_handle)
{
    switch_sflow_info_t *sflow_info = NULL;
    _switch_handle_get(switch_sflow_info_t, switch_sflow_array, sflow_handle, 
                        sflow_info);
    return sflow_info;
}

static switch_handle_t
switch_sflow_ace_handle_create()
{
    ...
    _switch_handle_create(SWITCH_HANDLE_TYPE_SFLOW_ACE,
                          ...
                          NULL, sflow_ace_handle);
    return sflow_ace_handle;
}

void
switch_sflow_ace_handle_delete(switch_handle_t sflow_ace_handle)
{
    _switch_handle_delete(switch_sflow_match_entry_t,
                          switch_sflow_ace_array,
                          sflow_ace_handle);
}

switch_sflow_match_entry_t *
switch_sflow_ace_entry_get(switch_handle_t sflow_ace_handle)
{
    switch_sflow_match_entry_t *sflow_ace_info = NULL;
    _switch_handle_get(switch_sflow_match_entry_t, switch_sflow_ace_array, 
                        sflow_ace_handle, 
                        sflow_ace_info);
                             ...
                             switch_api_sflow_session_info_t *api_sflow_info)
{
    ...
    if (api_sflow_info->collector_type == SFLOW_COLLECTOR_TYPE_CPU) {
        ...
        sflow_info->mirror_hdl = switch_api_mirror_session_create(device,
                                                            &api_mirror_info);
...
#else
    (void)device; (void)api_sflow_info;
    return SWITCH_API_INVALID_HANDLE;
#endif
}
...
#ifdef P4_SFLOW_ENABLE
switch_status_t
switch_sflow_match_key_from_tlv (unsigned int key_value_count,
...
switch_status_t
switch_sflow_match_entry_remove(switch_sflow_info_t *sflow_info, 
                             switch_handle_t entry_hdl)
{
    tommy_node* node = tommy_list_head(&sflow_info->match_list);
    while (node) {
        switch_sflow_match_entry_t* obj = 
                                    (switch_sflow_match_entry_t *) node->data;
                                 ...
                                 switch_handle_t *entry_hdl)
{
    ...
    // TBD - check if the match_spec is already used - callers responsibilty for now
    ...
    *entry_hdl = switch_sflow_ace_handle_create();
    ...
    if (direction == SWITCH_API_DIRECTION_INGRESS) {
        status = switch_pd_sflow_ingress_table_add(device, &match_key,
                                                    priority, sample_rate,
                                                    sflow_info,
                                                    match_entry);
        if (status != SWITCH_STATUS_SUCCESS) {
            goto error_return;
        ...
        tommy_list_insert_tail(&sflow_info->match_list, &match_entry->node,
                                                        match_entry);

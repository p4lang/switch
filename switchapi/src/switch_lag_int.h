...
#define SWITCH_LAG_ID_FROM_IFINDEX(ifindex) \
  (ifindex & (~(SWITCH_IFINDEX_TYPE_LAG << SWITCH_IFINDEX_PORT_WIDTH)))
...
#define SWITCH_IS_LAG_IFINDEX(ifindex) \
  ((ifindex >> SWITCH_IFINDEX_PORT_WIDTH) == SWITCH_IFINDEX_TYPE_LAG)
...
switch_status_t switch_lag_update_prune_mask_table(switch_device_t device,
                                                   switch_lag_info_t *lag_info);

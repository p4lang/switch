...
static inline void switch_mac_table_entry_key_init(uchar *key,
                                                   switch_handle_t vlan,
...
static inline int switch_mac_entry_hash_cmp(const void *key1,
                                            const void *key2) {
  return memcmp(key1, key2, 10);
}

static inline void switch_print_mac_table_entry(
    switch_api_mac_entry_t *mac_entry, char *buffer, int buffer_size) {
  ...
  snprintf(
      buffer,
      buffer_size,
      "type: %s vlan handle: %lx mac: %02x:%02x:%02x:%02x:%02x:%02x -> handle: "
      "%lx",
...
static switch_status_t switch_mac_insert_into_vlan_list(
  ...
  tommy_list_insert_tail(
      &(mac_vlan_list->mac_entries), &(mac_info->vlan_node), mac_info);
  mac_vlan_list->num_entries++;
  return SWITCH_STATUS_SUCCESS;
}

static switch_status_t switch_mac_insert_into_interface_list(
    switch_mac_info_t *mac_info) {
  ...
  tommy_list_insert_tail(
      &(mac_intf_list->mac_entries), &(mac_info->interface_node), mac_info);
  mac_intf_list->num_entries++;
  return SWITCH_STATUS_SUCCESS;
}

static switch_status_t switch_mac_remove_from_vlan_list(
    switch_mac_info_t *mac_info) {
  switch_mac_vlan_list_t *mac_vlan_list = NULL;
  switch_api_mac_entry_t *mac_entry = NULL;
  void *temp = NULL;
  ...
  mac_vlan_list = (switch_mac_vlan_list_t *)(*(unsigned long *)temp);
  tommy_list_remove_existing(&(mac_vlan_list->mac_entries),
                             &(mac_info->vlan_node));
  mac_vlan_list->num_entries--;
...
static switch_status_t switch_mac_remove_from_interface_list(
    switch_mac_info_t *mac_info) {
  switch_mac_intf_list_t *mac_intf_list = NULL;
  switch_api_mac_entry_t *mac_entry = NULL;
  void *temp = NULL;
  ...
  mac_intf_list = (switch_mac_intf_list_t *)(*(unsigned long *)temp);
  tommy_list_remove_existing(&(mac_intf_list->mac_entries),
                             &(mac_info->interface_node));
  mac_intf_list->num_entries--;
...
static switch_status_t switch_mac_table_entry_insert(
    switch_api_mac_entry_t *mac_entry, switch_mac_info_t **mac_info) {
  ...
  switch_mac_table_entry_key_init(
      key, mac_entry->vlan_handle, &(mac_entry->mac), &len, &hash);
  *mac_info = switch_malloc(sizeof(switch_mac_info_t), 1);
  ...
  tommy_hashtable_insert(
      &switch_mac_hash_table, &((*mac_info)->node), *mac_info, hash);
...
cleanup:
  SWITCH_API_ERROR(
      "%s:%d: unable to insert mac hash\n", __FUNCTION__, __LINE__);
  return status;
}

static switch_status_t switch_mac_table_entry_delete(
  ...
  switch_mac_table_entry_key_init(
      key, mac_entry->vlan_handle, &(mac_entry->mac), &len, &hash);
  mac_info = tommy_hashtable_remove(
      &switch_mac_hash_table, switch_mac_entry_hash_cmp, key, hash);
...
switch_mac_info_t *switch_mac_table_entry_find(
  ...
  switch_mac_table_entry_key_init(
      key, mac_entry->vlan_handle, &(mac_entry->mac), &len, &hash);
  mac_info = tommy_hashtable_search(
      &switch_mac_hash_table, switch_mac_entry_hash_cmp, key, hash);
  return mac_info;
}

#ifndef P4_L2_DISABLE
p4_pd_status_t switch_mac_learn_notify_cb(
  ...
  for (index = 0; index < msg->num_entries; index++) {
    ...
    SWITCH_API_TRACE(
        "%s:%d:MAC learn BD: 0x%d, MAC: 0x%02x:%02x:%02x:%02x:%02x:%02x => If: "
        "%d\n",
        __FUNCTION__,
        __LINE__,
    ...
    mac_entry.vlan_handle =
        id_to_handle(SWITCH_HANDLE_TYPE_BD, learn_entry->ingress_metadata_bd);
    bd_info = switch_bd_get(mac_entry.vlan_handle);
    if (!bd_info) {
      SWITCH_API_TRACE(
          "%s:%d: Ignoring the mac. vlan not found!", __FUNCTION__, __LINE__);
      continue;
    }

    if (!SWITCH_LN_LEARN_ENABLED(bd_info)) {
      SWITCH_API_TRACE("%s:%d: Ignoring the mac. learning disabled on vlan!",
                       __FUNCTION__,
                       __LINE__);
      continue;
    }

    intf_handle = switch_api_interface_get_from_ifindex(
        learn_entry->ingress_metadata_ifindex, mac_entry.vlan_handle);
    if (!intf_handle) {
      SWITCH_API_TRACE(
          "%s:%d: Ignoring the mac. invalid ifindex!", __FUNCTION__, __LINE__);
      continue;
    }

    intf_info = switch_api_interface_get(intf_handle);
    if (!intf_info) {
      SWITCH_API_TRACE("%s:%d: Ignoring the mac. invalid interface!",
                       __FUNCTION__,
                       __LINE__);
      continue;
    }
...
void switch_mac_aging_notify_cb(p4_pd_entry_hdl_t entry_hdl,
                                void *client_data) {
  switch_mac_info_t *mac_info = NULL;
  void *temp = NULL;
  switch_api_mac_entry_t *mac_entry = NULL;
  ...
  mac_entry = &(mac_info->mac_entry);
  SWITCH_API_TRACE(
      "%s:%d: Received aging notification %x - (%lx, "
      "0x%02x:%02x:%02x:%02x:%02x:%02x) -> %lx)",
      __FUNCTION__,
      __LINE__,
      entry_hdl,
      mac_entry->vlan_handle,
...
switch_api_mac_entry_t *switch_mac_aging_poll_entries(switch_device_t device) {
  switch_mac_info_t *mac_info = NULL;
  void *temp = NULL;
...
static switch_status_t switch_mac_update_nhop(
  ...
  neighbor_dmac =
      switch_neighbor_dmac_search_hash(mac_entry->vlan_handle, &mac_entry->mac);
  if (!neighbor_dmac) {
    return SWITCH_STATUS_SUCCESS;
  }
...
switch_status_t switch_api_mac_table_entry_add(
  ...
  switch (handle_type) {
    ...
    case SWITCH_HANDLE_TYPE_LAG:
      status =
          switch_interface_handle_get(mac_entry->handle, 0x0, &intf_handle);
      if (status != SWITCH_STATUS_SUCCESS) {
        goto cleanup;
      }
      intf_info = switch_api_interface_get(intf_handle);
  ...
  status = switch_pd_dmac_table_add_entry(device,
                                          mac_entry,
                                          nhop_index,
                                          mgid_index,
                                          aging_time,
...
switch_status_t switch_api_mac_table_entry_update(
...
#ifdef SWITCH_PD
  status = switch_pd_dmac_table_update_entry(device,
                                             &(mac_info->mac_entry),
                                             nhop_index,
                                             mgid_index,
                                             intf_info,
  ...
  // Do not learn multicast macs on smac table
  if (!mgid_index && SWITCH_LN_LEARN_ENABLED(bd_info)) {
    status = switch_pd_smac_table_update_entry(
        device, &(mac_info->mac_entry), intf_info, mac_info->smac_entry);
  }
  if (status != SWITCH_STATUS_SUCCESS) {
    goto cleanup;
  }
...
switch_status_t switch_api_mac_table_entry_delete(
  ...
  switch (handle_type) {
    case SWITCH_HANDLE_TYPE_PORT:
    case SWITCH_HANDLE_TYPE_LAG:
      status = switch_interface_handle_get(handle, 0x0, &intf_handle);
      if (status != SWITCH_STATUS_SUCCESS) {
        goto cleanup;
      }
      intf_info = switch_api_interface_get(intf_handle);
...
switch_status_t switch_api_mac_entries_delete(
    switch_device_t device,
    uint16_t mac_entry_count,
    switch_api_mac_entry_t *mac_entries) {
  switch_status_t status = SWITCH_STATUS_SUCCESS;
...
switch_status_t switch_api_mac_table_entries_delete_all(
    switch_device_t device) {
  switch_status_t status = SWITCH_STATUS_SUCCESS;
  ...
  SWITCH_API_TRACE(
      "%s:%d: Deleting all mac table entries\n", __FUNCTION__, __LINE__);

  JLF(temp, vlan_mac_hdl_array, vlan_handle);
  while (temp) {
    status = switch_api_mac_table_entries_delete_by_vlan(device, vlan_handle);
    JLN(temp, vlan_mac_hdl_array, vlan_handle);
  }
  return status;
}

switch_status_t switch_api_mac_table_entries_delete_by_vlan(
...
switch_status_t switch_api_mac_table_entries_delete_by_interface(
  ...
  if (!SWITCH_PORT_HANDLE_VALID(handle) && !SWITCH_LAG_HANDLE_VALID(handle) &&
  ...
  if (handle_type == SWITCH_HANDLE_TYPE_PORT ||
    ...
    status = switch_interface_handle_get(handle, 0x0, &intf_handle);
    if (status != SWITCH_STATUS_SUCCESS) {
      SWITCH_API_ERROR("mac table delete by interface failed");
      return status;
    }
  }
...
switch_status_t switch_api_mac_table_entries_delete_by_interface_vlan(
    switch_device_t device,
  ...
  if (handle_type == SWITCH_HANDLE_TYPE_PORT ||
    ...
    status = switch_interface_handle_get(handle, 0x0, &intf_handle);
    if (status != SWITCH_STATUS_SUCCESS) {
      SWITCH_API_ERROR("mac table delete by vlan failed");
      return status;
    }
  }
...
switch_status_t switch_api_mac_register_learning_callback(
    switch_mac_learn_entry_notify_cb cb_fn) {
  switch_mac_cb_fn.mac_learn_notify_cb = cb_fn;
  return SWITCH_STATUS_SUCCESS;
}

switch_status_t switch_api_mac_register_aging_callback(
    switch_mac_aging_entry_notify_cb cb_fn) {
  switch_mac_cb_fn.mac_aging_notify_cb = cb_fn;
  return SWITCH_STATUS_SUCCESS;
}

switch_status_t switch_api_mac_table_entries_get(
...
switch_status_t switch_api_mac_table_entries_get_by_vlan(
...
switch_status_t switch_api_mac_table_entries_get_by_interface(
    switch_handle_t intf_handle, switch_mac_table_iterator_fn iterator_fn) {
  switch_mac_info_t *mac_info = NULL;
  switch_mac_intf_list_t *mac_intf_list = NULL;
  tommy_node *node = NULL;
  void *temp = NULL;

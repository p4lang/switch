...
switch_api_id_allocator *switch_api_id_allocator_new(unsigned int initial_size,
                                                     bool zero_based) {
  switch_api_id_allocator *allocator =
      switch_malloc(sizeof(switch_api_id_allocator), 1);
...
unsigned int switch_api_id_allocator_allocate_contiguous(
    switch_api_id_allocator *allocator, uint8_t count) {
  unsigned int i;
  for (i = 0; i < allocator->n_words; i++) {
    if (allocator->data[i] != 0xFFFFFFFF) {
      ...
      if ((pos = _fit_width(allocator->data[i], count)) > 0) {
        ...
        return 32 * i + (32 - pos) + (allocator->zero_based ? 0 : 1);
      }
    }
  }
  uint32_t n_words = allocator->n_words;
  allocator->data =
      switch_realloc(allocator->data, n_words * 2 * sizeof(uint32_t));
...
unsigned int switch_api_id_allocator_allocate(
    switch_api_id_allocator *allocator) {
  return switch_api_id_allocator_allocate_contiguous(allocator, 1);
}

void switch_api_id_allocator_release(switch_api_id_allocator *allocator,
                                     unsigned int id) {
 ...
 * @param id id to set
 */
void switch_api_id_allocator_set(switch_api_id_allocator *allocator,
                                 unsigned int id) {
...
*/
int switch_api_id_allocator_is_set(switch_api_id_allocator *allocator,
                                   unsigned int id) {
  // assert (allocator != NULL);
  if (allocator->zero_based != true) {
    // assert(id > 0);
...
int id_main(int argc, char **argv) {
  ...
  unsigned int iter;
  switch_api_id_allocator *allocator =
      switch_api_id_allocator_new(INITIAL_WORDS, FALSE);

  for (i = 0; i < 40; i++) switch_api_id_allocator_allocate(allocator);

  printf("words are 0x%x, 0x%x\n",
         (uint32_t)allocator->data[0],
         (uint32_t)allocator->data[1]);

  for (i = 0; i < 40; i++) switch_api_id_allocator_release(allocator, i + 1);
  ...
  for (iter = 0; iter < MAX_ID_TEST; iter++) {
    for (i = 0; i < 1000; i++) switch_api_id_allocator_allocate(allocator);

    for (i = 0; i < 1000; i++) switch_api_id_allocator_release(allocator, i);
  }
...
#define BLOCK_SIZE 8
  for (i = 0; i < NUM_BLOCKS; i++) {
    unsigned int id =
        switch_api_id_allocator_allocate_contiguous(allocator, BLOCK_SIZE);

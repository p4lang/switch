...
switch_meter_info_t *switch_meter_info_get(switch_handle_t meter_handle) {
  switch_meter_info_t *meter_info = NULL;
  _switch_handle_get(
      switch_meter_info_t, switch_meter_array, meter_handle, meter_info);
  return meter_info;
}

switch_status_t switch_meter_delete(switch_handle_t meter_handle) {
  _switch_handle_delete(switch_meter_info_t, switch_meter_array, meter_handle);
  return SWITCH_STATUS_SUCCESS;
}
...
switch_handle_t switch_api_meter_create(switch_device_t device,
  ...
  if (api_meter_info->meter_mode == SWITCH_METER_MODE_STORM_CONTROL) {
    ...
    meter_info->api_meter_info.action[SWITCH_COLOR_YELLOW] =
        meter_info->api_meter_info.action[SWITCH_COLOR_GREEN];
    status = switch_pd_storm_control_meter_add_entry(
        device, handle_to_id(meter_handle), meter_info);
  } else {
    status =
        switch_pd_meter_index_table_add_entry(device,
  ...
  status = switch_pd_meter_action_table_add_entry(device,
                                                  handle_to_id(meter_handle),
                                                  meter_info,
...
switch_status_t switch_api_meter_update(switch_device_t device,
  ...
  if (api_meter_info->meter_mode == SWITCH_METER_MODE_STORM_CONTROL) {
    ...
    status =
        switch_pd_meter_index_table_update_entry(device,
                                                 handle_to_id(meter_handle),
                                                 meter_info,
                                                 meter_info->meter_idx_pd_hdl);
  }

  status = switch_pd_meter_action_table_update_entry(device,
                                                     handle_to_id(meter_handle),
                                                     meter_info,
                                                     meter_info->action_pd_hdl);
...
switch_status_t switch_api_meter_delete(switch_device_t device,
  ...
  status = switch_pd_meter_action_table_delete_entry(device,
                                                     meter_info->action_pd_hdl);
...
switch_status_t switch_api_meter_stats_get(switch_device_t device,
  ...
  int index = 0;
  switch_meter_stats_t counter_id = 0;

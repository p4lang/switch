...
switch_handle_t switch_lag_handle_create() {
  switch_handle_t lag_handle;
  _switch_handle_create(SWITCH_HANDLE_TYPE_LAG,
                        switch_lag_info_t,
                        switch_lag_array,
                        &info,
                        lag_handle);
...
switch_status_t switch_lag_handle_delete(switch_handle_t lag_handle) {
...
switch_handle_t switch_lag_member_handle_create() {
...
switch_status_t switch_lag_member_handle_delete(
    switch_handle_t lag_member_handle) {
  _switch_handle_delete(
      switch_lag_member_t, switch_lag_member_array, lag_member_handle);
  return SWITCH_STATUS_SUCCESS;
}

switch_lag_member_t *switch_api_lag_member_get_internal(
    switch_handle_t lag_member_handle) {
...
switch_status_t switch_api_lag_member_add(switch_device_t device,
  ...
  if (!lag_info->egr_bmap) {
    lag_info->egr_bmap =
        switch_api_id_allocator_new(SWITCH_INITIAL_LAG_SIZE, FALSE);
    if (!lag_info->egr_bmap) {
  ...
  switch (direction) {
    ...
    case SWITCH_API_DIRECTION_EGRESS:
      tommy_list_insert_head(
          &lag_info->egress, &(lag_member->egress_node), lag_member);
      ...
      if (lag_info->count == 0) {
        status = switch_pd_lag_group_table_add_entry_with_selector(
            device,
            lag_info->ifindex,
            lag_info->pd_group_hdl,
            &lag_info->hw_entry);
      ...
      if (direction == SWITCH_API_DIRECTION_EGRESS) break;
    case SWITCH_API_DIRECTION_INGRESS:
      tommy_list_insert_head(
          &lag_info->ingress, &(lag_member->ingress_node), lag_member);
      // lookup port table
      port_info = switch_api_port_get_internal(port);
      if (!port_info) {
        return SWITCH_STATUS_INVALID_PORT_NUMBER;
...
switch_status_t switch_api_lag_member_delete(switch_device_t device,
  ...
  switch (direction) {
    ...
    case SWITCH_API_DIRECTION_EGRESS:
      ...
      if (lag_info->count == 1) {
        status =
            switch_pd_lag_group_table_delete_entry(device, lag_info->hw_entry);
        if (status != SWITCH_STATUS_SUCCESS) {
          return status;
        }
      }
    ...
    case SWITCH_API_DIRECTION_INGRESS:
      ...
      lag_member =
          tommy_list_remove_existing(&(lag_info->ingress), delete_node);
      port_info = switch_api_port_get_internal(port);
      if (!port_info) {
        return SWITCH_STATUS_INVALID_PORT_NUMBER;
      }
...
switch_handle_t switch_api_lag_member_create(switch_device_t device,
  ...
  status = switch_api_lag_member_add(device, lag_handle, direction, port);
...
switch_status_t switch_api_lag_member_remove(
  ...
  status = switch_api_lag_member_delete(
      device, lag_member->lag_handle, lag_member->direction, lag_member->port);
  return status;
}
...
switch_status_t switch_api_lag_print_entry(switch_handle_t lag_handle) {
  switch_lag_info_t *lag_info = NULL;
  switch_lag_member_t *lag_member = NULL;
  tommy_node *node = NULL;
...
switch_status_t switch_api_lag_print_all(void) {

...
switch_handle_t switch_nhop_create() {
  ...
  _switch_handle_create(SWITCH_HANDLE_TYPE_NHOP,
                        switch_nhop_info_t,
                        switch_nhop_array,
                        NULL,
                        nhop_handle);
...
switch_nhop_info_t *switch_nhop_get(switch_handle_t nhop_handle) {
  switch_nhop_info_t *nhop_info = NULL;
  _switch_handle_get(
      switch_nhop_info_t, switch_nhop_array, nhop_handle, nhop_info);
  return nhop_info;
}
...
static inline void switch_nhop_hash_key_init(uchar *key,
                                             switch_nhop_key_t *nhop_key,
                                             uint32_t *len,
                                             uint32_t *hash) {
  *len = 0;
...
static inline int switch_nhop_hash_cmp(const void *key1, const void *key2) {
  return memcmp(key1, key2, SWITCH_NHOP_HASH_KEY_SIZE);
}

static switch_status_t switch_nhop_insert_hash(switch_spath_info_t *spath_info,
                                               switch_nhop_key_t *nhop_key,
                                               switch_handle_t nhop_handle) {
  ...
  tommy_hashtable_insert(
      &switch_nhop_hash_table, &(spath_info->node), spath_info, hash);
  return SWITCH_STATUS_SUCCESS;
}

static switch_status_t switch_nhop_delete_hash(
  ...
  switch_nhop_hash_key_init(key, temp_nhop_key, &len, &hash);
  spath_info = tommy_hashtable_remove(
      &switch_nhop_hash_table, switch_nhop_hash_cmp, key, hash);
  if (!spath_info) {
...
switch_handle_t switch_api_nhop_handle_get(switch_nhop_key_t *nhop_key) {
  ...
  spath_info = tommy_hashtable_search(
      &switch_nhop_hash_table, switch_nhop_hash_cmp, key, hash);
  if (!spath_info) {
...
switch_status_t switch_nhop_ifindex_get(switch_handle_t nhop_handle,
  ...
  *mc_index = 0;
  ...
  if (SWITCH_INTF_TYPE(intf_info) == SWITCH_API_INTERFACE_L3_VLAN) {
    ...
    if (neighbor_handle == SWITCH_API_INVALID_HANDLE || neighbor_handle == 0) {
...
switch_handle_t switch_api_nhop_create(switch_device_t device,
  ...
  if ((nhop_handle = switch_api_nhop_handle_get(nhop_key)) ==
  ...
  if (SWITCH_INTF_IS_PORT_L3(intf_info) && intf_info->bd_handle) {
    status =
        switch_pd_urpf_bd_table_add_entry(device,
                                          handle_to_id(nhop_handle),
                                          handle_to_id(intf_info->bd_handle),
...
switch_status_t switch_api_nhop_update(switch_device_t device,
  ...
  bool tunnel = (SWITCH_INTF_TYPE(intf_info) == SWITCH_API_INTERFACE_TUNNEL);
  status =
      switch_pd_nexthop_table_update_entry(device,
                                           handle_to_id(nhop_handle),
                                           handle_to_id(intf_info->bd_handle),
                                           ifindex,
                                           flood,
...
switch_status_t switch_api_nhop_delete(switch_device_t device,
  ...
  if (nhop_info->u.spath.neighbor_handle == 0) {
    ...
    if (SWITCH_INTF_IS_PORT_L3(intf_info) && intf_info->bd_handle) {
      status = switch_pd_urpf_bd_table_delete_entry(device,
                                                    spath_info->urpf_hw_entry);
      if (status != SWITCH_STATUS_SUCCESS) {
        return status;
      }
...
#ifdef SWITCH_PD
  status = switch_pd_ecmp_group_table_add_entry_with_selector(
      device,
      handle_to_id(ecmp_handle),
      ecmp_info->pd_group_hdl,
      &(ecmp_info->hw_entry));
...
switch_status_t switch_api_ecmp_member_delete(
  ...
  for (count = 0; count < nhop_count; count++) {
    ...
    if (ecmp_info->count == 1) {
      status =
          switch_pd_ecmp_group_table_delete_entry(device, ecmp_info->hw_entry);
      if (status != SWITCH_STATUS_SUCCESS) {
        return status;
      }
    }
    status = switch_pd_ecmp_member_delete(
        device, ecmp_info->pd_group_hdl, ecmp_member->mbr_hdl);
    if (status != SWITCH_STATUS_SUCCESS) {
      return status;
    }
    if (SWITCH_INTF_IS_PORT_L3(intf_info) && intf_info->bd_handle) {
...
switch_status_t switch_api_nhop_print_entry(switch_handle_t nhop_handle) {
  switch_nhop_info_t *nhop_info = NULL;
  switch_spath_info_t *spath_info = NULL;
  switch_ecmp_info_t *ecmp_info = NULL;
  switch_ecmp_member_t *ecmp_member = NULL;

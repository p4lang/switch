...
switch_handle_t switch_vrf_create() {
  switch_handle_t handle;
  _switch_handle_create(SWITCH_HANDLE_TYPE_VRF,
                        switch_vrf_info_t,
                        switch_vrf_array,
                        NULL,
                        handle);
...
switch_vrf_info_t *switch_vrf_get(switch_handle_t handle) {
  switch_vrf_info_t *vrf_info = NULL;
  _switch_handle_get(switch_vrf_info_t, switch_vrf_array, handle, vrf_info);
  return vrf_info;
}

switch_handle_t switch_api_vrf_create(switch_device_t device,
...
switch_status_t switch_api_vrf_delete(switch_device_t device,
                                      switch_handle_t handle) {
  switch_vrf_info_t *vrf_info = NULL;
...
switch_status_t switch_api_vrf_type_set(switch_handle_t vrf_handle,
                                        uint64_t value) {
  switch_vrf_info_t *vrf_info = NULL;
...
switch_status_t switch_api_vrf_type_get(switch_handle_t vrf_handle,
                                        uint64_t *value) {
  switch_vrf_info_t *vrf_info = NULL;

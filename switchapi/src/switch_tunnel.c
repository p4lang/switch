...
switch_status_t switch_tunnel_init(switch_device_t device) {
  ...
  src_vtep_index_allocator =
      switch_api_id_allocator_new(SWITCH_SRC_VTEP_HASH_TABLE_SIZE, FALSE);
  dst_vtep_index_allocator =
      switch_api_id_allocator_new(SWITCH_DST_VTEP_HASH_TABLE_SIZE, FALSE);
...
static inline int switch_vtep_hash_cmp(const void *key1, const void *key2) {
...
static uint16_t switch_tunnel_src_vtep_insert_hash(
  ...
  tommy_hashtable_insert(
      &switch_src_vtep_table, &(vtep_entry->node), vtep_entry, hash);
  return src_vtep_index;
}
...
static switch_status_t switch_tunnel_src_vtep_delete_hash(
  ...
  switch_api_id_allocator_release(src_vtep_index_allocator,
                                  vtep_entry->entry_index);
  free(vtep_entry);
  return status;
}
...
static uint16_t switch_tunnel_dst_vtep_insert_hash(
  ...
  tommy_hashtable_insert(
      &switch_dst_vtep_table, &(vtep_entry->node), vtep_entry, hash);
  return dst_vtep_index;
}
...
static switch_status_t switch_tunnel_dst_vtep_delete_hash(
  ...
  switch_api_id_allocator_release(dst_vtep_index_allocator,
                                  vtep_entry->entry_index);
  free(vtep_entry);
  return status;
}
...
static switch_status_t switch_tunnel_ip_encap_table_add_entries(
  ...
  if (intf_info->direction == SWITCH_API_DIRECTION_EGRESS ||
    ...
    if (status != SWITCH_STATUS_SUCCESS) {
      SWITCH_API_ERROR(
          "%s:%d: unable to add src rewrite entry for "
          "interface %lx",
          __FUNCTION__,
          __LINE__,
          intf_handle);
    ...
    if (status != SWITCH_STATUS_SUCCESS) {
      SWITCH_API_ERROR(
          "%s:%d: unable to add dst rewrite entry for "
          "interface %lx",
          __FUNCTION__,
          __LINE__,
          intf_handle);
...
static switch_status_t switch_tunnel_ip_encap_table_delete_entries(
    switch_device_t device,
    switch_handle_t intf_handle,
    switch_interface_info_t *intf_info) {
  switch_ip_encap_t *ip_encap = NULL;
  ...
  if (intf_info->direction == SWITCH_API_DIRECTION_BOTH ||
    ...
    if (status != SWITCH_STATUS_SUCCESS) {
      SWITCH_API_ERROR(
          "%s:%d: unable to delete src rewrite entry for "
          "interface %lx",
          __FUNCTION__,
          __LINE__,
          intf_handle);
    ...
    if (status != SWITCH_STATUS_SUCCESS) {
      SWITCH_API_ERROR(
          "%s:%d: unable to delete dst rewrite entry for "
          "interface %lx",
          __FUNCTION__,
          __LINE__,
          intf_handle);
...
static uint32_t switch_tunnel_mpls_get_pop_label(
...
static switch_status_t switch_tunnel_mpls_table_add_entries(
    switch_device_t device,
  ...
  switch (mpls_encap->mpls_mode) {
    ...
    case SWITCH_API_MPLS_TRANSIT:
      ...
      status = switch_pd_mpls_table_add_entry(device,
                                              mpls_encap,
    ...
    default:
      SWITCH_API_ERROR(
          "%s:%d invalid mpls tunnel mode!", __FUNCTION__, __LINE__);
      status = SWITCH_STATUS_UNSUPPORTED_TYPE;
  }
  return status;
}

static switch_status_t switch_tunnel_mpls_table_delete_entries(
    switch_device_t device,
    p4_pd_entry_hdl_t entry_hdl,
    switch_mpls_encap_t *mpls_encap) {
  switch_status_t status = SWITCH_STATUS_SUCCESS;
...
switch_handle_t switch_api_tunnel_interface_create(
    switch_device_t device,
  ...
  if (!intf_info) {
    SWITCH_API_ERROR(
        "%s:%d: unable to allocate memory!", __FUNCTION__, __LINE__);
  ...
  if (status != SWITCH_STATUS_SUCCESS) {
    SWITCH_API_ERROR(
        "%s:%d: failed to create tunnel interface!", __FUNCTION__, __LINE__);
    return SWITCH_API_INVALID_HANDLE;
  }
  return intf_handle;
}
...
switch_status_t switch_api_tunnel_interface_delete(
  ...
  if (tunnel_info->encap_mode == SWITCH_API_TUNNEL_ENCAP_MODE_IP) {
    status = switch_tunnel_ip_encap_table_delete_entries(
        device, intf_handle, intf_info);
  }
  ...
  SWITCH_API_TRACE("%s:%d: Tunnel interface %lx deleted",
                   __FUNCTION__,
                   __LINE__,
                   intf_handle);
cleanup:
...
switch_status_t switch_api_logical_network_member_add(
  ...
  switch (SWITCH_LN_NETWORK_TYPE(bd_info)) {
    case SWITCH_LOGICAL_NETWORK_TYPE_ENCAP_BASIC:
      status = switch_api_logical_network_member_add_basic(
          device, bd_handle, intf_handle);
      break;
    case SWITCH_LOGICAL_NETWORK_TYPE_ENCAP_ENHANCED:
      status = switch_api_logical_network_member_add_enhanced(
          device, bd_handle, intf_handle);
      break;
...
switch_status_t switch_api_logical_network_member_add_basic(
    switch_device_t device,
    switch_handle_t bd_handle,
    switch_handle_t intf_handle) {
  switch_interface_info_t *intf_info = NULL;
  ...
  if (SWITCH_INTF_TYPE(intf_info) == SWITCH_API_INTERFACE_TUNNEL) {
    ...
    switch (SWITCH_INTF_TYPE(intf_info)) {
      ...
      default:
        SWITCH_API_ERROR(
            "%s:%d: trying to add unsupported interface type for ln %lx",
            __FUNCTION__,
            __LINE__,
            bd_handle);
        return SWITCH_STATUS_UNSUPPORTED_TYPE;
    ...
    status =
        switch_pd_port_vlan_mapping_table_add_entry(device,
                                                    vlan_id,
                                                    0,
                                                    intf_info,
                                                    bd_info->bd_entry,
    ...
    if (status != SWITCH_STATUS_SUCCESS) {
      SWITCH_API_ERROR(
          "%s:%d: unable to add port-vlan entry for interface %lx ln %lx",
          __FUNCTION__,
          __LINE__,
          intf_handle,
          bd_handle);
  ...
  if (status != SWITCH_STATUS_SUCCESS) {
    SWITCH_API_ERROR(
        "%s:%d: Unable to add interface %lx to flood list of ln %lx",
        __FUNCTION__,
        __LINE__,
        intf_handle,
        bd_handle);
...
switch_status_t switch_api_logical_network_member_add_enhanced(
    switch_device_t device,
    switch_handle_t bd_handle,
    switch_handle_t intf_handle) {
  switch_interface_info_t *intf_info = NULL;
  ...
  if (SWITCH_INTF_TYPE(intf_info) == SWITCH_API_INTERFACE_TUNNEL) {
    ...
    if (tunnel_info->encap_mode == SWITCH_API_TUNNEL_ENCAP_MODE_IP) {
      ...
      encap_info = &(SWITCH_INTF_TUNNEL_ENCAP_INFO(intf_info));
      tunnel_vni = switch_tunnel_get_tunnel_vni(encap_info);
      tunnel_type = switch_tunnel_get_egress_tunnel_type(encap_info->encap_type,
                                                         ip_encap);
      ...
      status = switch_tunnel_mpls_table_add_entries(
          device, bd_handle, ln_member->tunnel_hw_entry, mpls_encap);
      if (status != SWITCH_STATUS_SUCCESS) {
        SWITCH_API_ERROR(
            "%s:%d unable to add mpls entry!", __FUNCTION__, __LINE__);
        goto cleanup;
      }
    }
#endif
    ...
    if (status != SWITCH_STATUS_SUCCESS) {
      SWITCH_API_ERROR(
          "%s:%d: unable to add port to vlan!.", __FUNCTION__, __LINE__);
      return status;
    }
    status = switch_api_vlan_xlate_add(device, bd_handle, intf_handle, vlan_id);
#endif
...
switch_status_t switch_api_logical_network_member_remove_basic(
    switch_device_t device,
    switch_handle_t bd_handle,
    switch_handle_t intf_handle) {
  switch_interface_info_t *intf_info = NULL;
  ...
  if (SWITCH_INTF_TYPE(intf_info) == SWITCH_API_INTERFACE_TUNNEL) {
    ...
    status = switch_pd_port_vlan_mapping_table_delete_entry(
        device, ln_member->pv_hw_entry);
#ifdef SWITCH_PD
    if (status != SWITCH_STATUS_SUCCESS) {
      SWITCH_API_ERROR(
          "%s:%d: unable to delete port-vlan entry for interface %lx ln %lx",
          __FUNCTION__,
          __LINE__,
          intf_handle,
          bd_handle);
    ...
    status =
        switch_api_vlan_xlate_remove(device, bd_handle, intf_handle, vlan_id);
  }
  if (SWITCH_INTF_FLOOD_ENABLED(intf_info)) {
    switch_vlan_interface_t vlan_intf;
    memset(&vlan_intf, 0, sizeof(vlan_intf));
...
switch_status_t switch_api_logical_network_member_remove_enhanced(
    switch_device_t device,
    switch_handle_t bd_handle,
    switch_handle_t intf_handle) {
  switch_interface_info_t *intf_info = NULL;
  ...
  if (SWITCH_INTF_TYPE(intf_info) == SWITCH_API_INTERFACE_TUNNEL) {
    ...
    if (tunnel_info->encap_mode == SWITCH_API_TUNNEL_ENCAP_MODE_IP) {
      ...
      mpls_encap = &(SWITCH_INTF_TUNNEL_MPLS_ENCAP(intf_info));
      status = switch_tunnel_mpls_table_delete_entries(
          device, ln_member->tunnel_hw_entry[0], mpls_encap);
    }
#endif
  } else {
    switch (SWITCH_INTF_TYPE(intf_info)) {
  ...
  if (SWITCH_INTF_FLOOD_ENABLED(intf_info)) {
    ...
    if (status != SWITCH_STATUS_SUCCESS) {
      SWITCH_API_ERROR(
          "%s:%d: Unable to remove interface %lx from flood list of ln %lx",
          __FUNCTION__,
          __LINE__,
          intf_handle,
          bd_handle);
...
switch_status_t switch_api_logical_network_member_remove(
  ...
  switch (SWITCH_LN_NETWORK_TYPE(bd_info)) {
    case SWITCH_LOGICAL_NETWORK_TYPE_ENCAP_BASIC:
      status = switch_api_logical_network_member_remove_basic(
          device, bd_handle, intf_handle);
      break;
    case SWITCH_LOGICAL_NETWORK_TYPE_ENCAP_ENHANCED:
      status = switch_api_logical_network_member_remove_enhanced(
          device, bd_handle, intf_handle);
      break;
    default:
      status = SWITCH_STATUS_INVALID_LN_TYPE;
...
switch_tunnel_type_ingress_t switch_tunnel_get_ingress_tunnel_type(
...
switch_tunnel_type_egress_t switch_tunnel_get_egress_tunnel_type(
    switch_encap_type_t encap_type, switch_ip_encap_t *ip_encap) {
...
switch_status_t switch_api_mpls_tunnel_transit_create(
    switch_device_t device, switch_mpls_encap_t *mpls_encap) {
  ...
  if (!temp) {
    ...
    *(unsigned long *)temp = (unsigned long)mpls_info;
    status = switch_tunnel_mpls_table_add_entries(
        device, 0, &mpls_info->tunnel_hw_entry, mpls_encap);
  } else {
...
switch_status_t switch_api_mpls_tunnel_transit_delete(
    switch_device_t device, switch_mpls_encap_t *mpls_encap) {
  switch_mpls_info_t *mpls_info = NULL;
  void *temp = NULL;
  uint32_t swap_label = 0;
  ...
  status = switch_tunnel_mpls_table_delete_entries(
      device, mpls_info->tunnel_hw_entry, mpls_encap);

...
typedef enum {
  ...
  SWITCH_HANDLE_TYPE_ECMP_MEMBER,
  SWITCH_HANDLE_TYPE_STP_PORT,
  SWITCH_HANDLE_TYPE_HOSTIF_TRAP,
...
#define _switch_handle_get(_info, _judy, _handle, _handle_info) \
  ...
  if (_gp) _handle_info = (_info *)(*(unsigned long *)_gp);

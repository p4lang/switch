...
parser.add_argument("--oft-path", required=True,
                    help="Location of oftest framework oft")
args, unknown_args = parser.parse_known_args()

root_dir = os.path.dirname(os.path.realpath(__file__))
oft_path = args.oft_path
test_dir = os.path.join(root_dir, "of-tests")

if __name__ == "__main__":
    new_args = unknown_args
    new_args += ["-S 127.0.0.1", "-V1.3"]
    new_args += ["--test-dir", test_dir]
